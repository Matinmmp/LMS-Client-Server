...
export const isAuthenticated = CatchAsyncError(async (req: Request, res: Response, next: NextFunction) => {
    ...
    const decode = jwt.verify(access_token, process.env.ACCESS_TOKEN as string) as { id: string };
    if (!decode) {
        return next(new ErrorHandler("توکن معتبر نیست", 401));
    }

    // ✅ دریافت اطلاعات یوزر از دیتابیس
    const user = await userModel.findById(decode.id); // حذف فیلد پسورد برای امنیت بیشتر

    if (!user) {
        return next(new ErrorHandler("لطفا وارد حساب خود شوید", 401));
    }

    // ✅ ذخیره اطلاعات یوزر در `req.user`
    req.user = user;
